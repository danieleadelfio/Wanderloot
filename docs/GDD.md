...
- Exp guadagnata raccogliendo le gemme lasciate dai nemici (da M5; prima era immediata) → level-up. Curva in `LevelCurve` (`data/run/level_curve.tres`): exp per passare da N a N+1 = `500 * 1.35^(N-1)` arrotondato (500, 675, 911, 1230, 1661… — riscalato x100 in M12 #86). L'exp in eccesso passa al livello successivo; più level-up in un colpo sono gestiti uno alla volta.
- Ad ogni level-up: pausa, 3 scelte casuali (pesate) tra potenziamenti d'arma, abilità passive, statistiche. **Reroll (M12, #86)**: bottone che rimescola le scelte con una in meno (minimo 1), scritto sul bottone; max 5 reroll per run (`RunManager.MAX_REROLLS`), condivisi da tutti i level-up della run.
  - **Stato M1**: 5 upgrade di statistica in `data/upgrades/` (Potenza +10% danno, Raffica +20% cadenza, Gittata +20% velocità proiettili, Agilità +10% movimento, Vigore +10% HP max e cura la stessa quantità), tutti con peso 1, estratti senza ripetizioni da `UpgradeTable`. Scelta con mouse o tastiera/pad (focus sul primo). Abilità passive/nuove armi: dopo l'MVP.
  - **Stato M5 (#25)**: 8 potenziamenti in più (13 totali), tutti cumulabili senza tetto. Statistiche: Persistenza +25% durata proiettili, Magnete +10% raggio di raccolta, Saggezza +15% exp raccolta, Fortuna +15% probabilità di drop, Guardia +0,2s di invulnerabilità, Impatto +30% spinta. Meccaniche (peso 0,6, più rari): Ventaglio +1 proiettile per colpo (ventaglio centrato, 10° tra i proiettili, compresso se supera 360°), Perforazione +1 nemico attraversato. Nuove voci di `UpgradeData.Stat` sempre in coda all'enum (i `.tres` salvano l'indice).
...
- **Obiettivo della run** (`first_run`): al primo `Arena._ready()` di sempre, spiega l'obiettivo in una frase (combattere, raccogliere loot, superare gli eventi, l'estrazione che si apre dopo un po' e il rischio di perdere tutto morendo prima di estrarre). Non ripete i tempi esatti (gia' mostrati dal countdown/indicatore di estrazione a schermo): resta concettuale. **Come gli eventi (M12, #86)**: schermo intero in pausa (`FirstTimeNotice`, non il solo `HUD.announce` a scomparsa), resta finche' non si preme Continua — prima spariva da sola dopo 7s, a volte prima ancora di essersi mossi.
- **Evento** e **avviso di overtime in arrivo** (`first_overtime_warning`, M12 #86, rivisto dopo feedback): a differenza degli altri annunci (`HUD.announce`, sparisce da solo mentre la run continua), la primissima volta la run **si ferma davvero**: `FirstTimeNotice` (`scenes/ui/FirstTimeNotice/`, `process_mode` ALWAYS come `LevelUpChoice`/`AbilityChoice`) mostra la spiegazione a schermo intero e resta in pausa finche' non si preme **Continua** — cosi' si legge con calma invece di rischiare di essere colpiti mentre si legge un banner a scomparsa. Per l'evento la chiave e' **per tipo di evento** (`event_<id>`, non un flag unico): la prima volta che capita un tipo mai incontrato prima si ferma di nuovo, non solo alla primissima run (M12 #86, rivisto dopo feedback); titolo e corpo sono lo stesso testo del Codex (obiettivo + ricompensa) per non duplicarlo. Titolo/sottotitolo dell'evento (`_hud.show_event`) restano visibili come sempre sotto. Dalla seconda volta in poi per quel tipo di evento (e nelle run successive) resta il solo annuncio normale a scomparsa.
...
- **Vita base del player: 1000** (da M11.3, #73; prima 5; riscalato x100 in M12 #86 insieme a danno/HP/exp in tutto il gioco).
- **Mana (M12, #86)**: pool consumato dallo sparo base (non dalle abilita' della bacchetta), rigenerato in continuo. Base **30 mana**, rigenerazione **6/s** (`PlayerStats.max_mana`/`mana_regen`); la Bacchetta base costa **1,5 mana a colpo** (`WeaponData.mana_cost`, 0 = nessun costo, retrocompatibile con le armi esistenti), quindi al ritmo base (4 colpi/s) la rigenerazione regge esattamente il consumo — il pool si svuota solo se la cadenza sale oltre la rigenerazione (upgrade Raffica, equip), obbligando a brevi pause. Nuovo upgrade di run **Meditazione** (+2 rigenerazione/s, `UpgradeData.Stat.MANA_REGEN`); solo upgrade per ora, non ancora un affix dell'equip (valutare in futuro se serve). A corto di mana il colpo resta "in credito" (non si scarica gratis, non si perde): riparte al primo tick in cui il pool torna sufficiente. Barra dedicata in HUD sotto la vita (`Hud.set_mana()`), letta ogni frame da `Player.mana` (RefCounted puro come `Knockback`, nessun nodo/segnale). Logica pura in `scripts/combat/mana.gd`, testata; gating in `Weapon.try_fire()`, testato.
- **Veleno (M11.3, #73)**: alcuni proiettili nemici avvelenano (`WeaponData` gruppo *Veleno*): **100 danno ogni 1,5 s per 4,5 s** (300 danni in tutto — riscalato x100 in M12 #86). Un nuovo colpo rinnova la durata senza sommare il danno; il player diventa verde finché dura. Logica in `PoisonState` (testata), nodo `Poison` sul player; a inizio run si azzera.
...
Slot: **Testa, Guanti, Armatura, Pantaloni, Stivali, Anello (×2), Amuleto** e **Bacchetta** (l'arma, che porta anche gli slot delle abilità). Gli oggetti esistenti passano agli slot nuovi: bacchette → Bacchetta, Stivali viscosi → Stivali, Amuleto del nucleo → Amuleto. Nell'inventario dell'hub un **manichino** mostra gli slot con l'icona dell'oggetto indossato: **clic sinistro su un oggetto** del baule → va nel suo slot (sostituendo quello presente); **clic sinistro su uno slot occupato** → l'oggetto torna nel baule. **Clic destro** su un'icona del baule (M12, #86) → segna/dissegna l'oggetto come **spazzatura**: un cestino semitrasparente compare in basso a sinistra sull'icona, nessun effetto automatico, serve solo a raggrupparli per lo smontaggio in blocco dal fabbro; persiste col salvataggio (`ItemInstance.is_trash`). Suggerimento sinistro/destro richiamato in calce all'inventario. I due anelli si riempiono in ordine (il secondo clic su un anello va nello slot libero, poi sostituisce il primo). Nuovi oggetti base: Cappuccio del viandante (Testa), Guanti del fabbro (Guanti), Corazza d'osso (Armatura), Brache di cuoio (Pantaloni), Anello di gelatina (Anello), ognuno con ricetta; gli Stivali viscosi passano allo slot Stivali. Il bordo degli slot e degli oggetti prende il colore della rarità (§6.2).
...
- Tabella per arena in `ArenaData.item_drops` (`data/equipment/drops_<arena>.tres`, `ItemDropTable`): oggetti possibili, `drop_chance` per uccisione (Cripta 0,267 %, Ossario 0,4 %, × bonus drop; ridotto di 1/3 in M12 #86, era 0,4 %/0,6 %), `max_tier` (4 = Leggendario), `boss_drops` (1 per boss) con `boss_min_tier` (2 = Raro). La rarità si tira coi pesi `drop_weight` di `rarity_table.tres`; bonus e abilità si tirano quando l'oggetto cade.
...
- **Fusione dal fabbro**: sei oggetti **identici** (stesso oggetto base) della **stessa rarità** → uno della rarità successiva, con bonus e modificatore ritirati. Si sale fino a Leggendario; il Mitico non si ottiene per fusione. (M12, #86: prima ne bastavano due, poi tre; M13, #86: ora sei.)
- **Smontaggio** di un oggetto in materiali (quantità crescente con la rarità), per gestire il baule. **In lavorazione (#60):** non deve restituire i materiali che si trovano in run (se ne raccolgono centinaia); darà materiali **ottenibili solo smontando**, ingredienti di altre ricette (quali, da definire). Per ora la scheda mostra gli oggetti con la scritta *Work in progress* e i pulsanti disattivati (`Blacksmith.salvage_enabled = false`); la logica in `Forge` resta, con la resa provvisoria descritta sotto. **Smonta tutta la spazzatura** (M12, #86): bottone sopra la lista che smonta in un colpo solo tutti gli oggetti del baule taggati spazzatura (clic destro, sopra), sommandone la resa — evita di dover smontare uno per uno; disattivato senza oggetti taggati o mentre lo smontaggio è in lavorazione (`Forge.salvage_trash()`, `MetaProgression.salvage_all_trash()`).
- Il fabbro ha tre schede: **Crafting**, **Fusione**, **Smontaggio**. Fusione e smontaggio valgono solo per oggetti nel baule (non equipaggiati). La fusione elenca le sestine disponibili (stesso oggetto, stessa rarità, sotto Leggendario, `Forge.FUSION_COUNT = 6`) e fonde le prime sei; il risultato è un oggetto nuovo tirato alla rarità successiva. Lo smontaggio rende `max(1, floor(costo ricetta × 25 % × salvage_multiplier))` per ogni materiale della ricetta (Comune ×1, Non comune ×2, Raro ×3, Super raro ×5, Leggendario ×8, Mitico ×12); es. Bacchetta di gelatina (75 Gelatina, M13 #86: costi di crafting triplicati): Comune 18, Leggendario 150. Lo smontaggio chiede conferma (secondo clic). Logica in `scripts/meta/forge.gd`.

...
- Input map: `move_*` (WASD, frecce, stick sinistro), `aim_*` (stick destro), `shoot` (mouse sinistro), `menu` (ESC, Start), `pause` (P).
- Pausa (M5, #23): ESC apre il menu di pausa (Riprendi, Pausa; predisposto per altre voci), P mette direttamente in pausa con la scritta PAUSA bianca semitrasparente al centro (96px su base 720p). ESC chiude qualsiasi pausa aperta. Logica in `PauseState` (pura, testata), input in `PauseController`; `Arena` mette in pausa se lo chiede la run (level-up, fine run) o il giocatore. Durante level-up e fine run ESC/P sono ignorati.

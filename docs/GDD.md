...
- **Backlog M12/M13 richiesto dal proprietario (#86, non ancora progettato in dettaglio)**: ~~evento "Scheletri nell'armadio"~~ → implementato, vedi §3.4; ~~sistema di potenza dell'equipaggiamento → livello arena~~ → implementato, vedi §6.3b; ~~shader di movimento ambientale~~ → implementato, vedi §8; tutorial contestuale strutturato (hub alla prima nuova partita, primi eventi, prima estrazione, primo overtime, tutti skippabili). Ognuno merita una progettazione propria (parametri esatti, issue dedicata) prima dell'implementazione.

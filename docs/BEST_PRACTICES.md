...
- **Movimento e interpolazione (M10.1)**: posizioni e rotazioni si cambiano solo in `_physics_process` (il rendering interpola). Ogni spostamento istantaneo (oggetto preso dal pool, teletrasporto, posizione caricata) chiama `reset_physics_interpolation()` subito dopo, altrimenti per un frame il nodo "scivola" dalla posizione vecchia.

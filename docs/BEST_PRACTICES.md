...
- **Build esportata = verità, non l'editor** (#85). All'export l'editor converte i `.tres` in binario da istanze segnaposto (script non `@tool`) e alcuni valori possono sparire solo nella build (successo con `event_times`: eventi mai partiti). Regole: (1) una classe Resource con un `Packed*Array` esportato è `@tool` (lo verifica `tests/data/test_export_safe_resources.gd`); (2) dopo aver cambiato una classe dati, prima di esportare cancellare `.godot/exported/` (la cache di conversione non si invalida quando cambia lo script); (3) prima di un rilascio provare nella build esportata almeno una run completa: eventi ai tempi fissi, estrazione, boss dopo l'estrazione, overtime.
...
- **Oggetti come istanze (M11)**: l'equipaggiamento posseduto si maneggia solo come `ItemInstance` per `uid` (mai per id dell'oggetto base, che può avere più copie); `EquipmentData` è il modello base e resta read-only.

...
func set_minimap_context(player: Node2D, indicators: MapIndicators, extraction_point: Node2D, pickup_pool: PickupPool = null, event_director: RunEventDirector = null) -> void:
	_minimap.setup(player, indicators, extraction_point, pickup_pool, event_director)
